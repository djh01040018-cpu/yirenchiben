...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QProgressBar, QSplitter, QFrame, QCheckBox, QScrollArea, QLineEdit, QMenu, QDialog, QFormLayout, QListWidgetItem, QSlider, QRadioButton, QButtonGroup)
...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QSize, QEvent, QCoreApplication
from PyQt6.QtGui import QFont, QColor, QTextCharFormat, QColor as RGBColor, QIcon, QFontDatabase, QScreen, QFontMetrics, QRawFont, QUndoCommand, QUndoStack, QKeySequence, QAction
...
class CollationEngine:
    ...
    # 1. 調用新引擎 (不會崩潰)
        ...
        def flush_buffer():
            ...
            # 1. 提取原始文本
            base_items = [(x['base'], x['base_idx']) for x in buffer if x['base'] != '-']
            wit_items = [(x['wit'], x['wit_idx']) for x in buffer if x['wit'] != '-']
            base_str = "".join([x[0] for x in base_items])
            wit_str = "".join([x[0] for x in wit_items])
            ...
            if common_total >= 2:
                base_common_ids = []   
                base_common_chars = [] 
                noise_base_items = []  # 改為存儲 (char, idx) 元組
                
                temp_int_base = intersection.copy()
                for (ch, b_idx), vid in zip(base_items, base_ids):
                    if temp_int_base[vid] > 0:
                        base_common_ids.append(vid)
                        base_common_chars.append(ch)
                        temp_int_base[vid] -= 1
                    else:
                        noise_base_items.append((ch, b_idx))
                        
                wit_common_ids = []
                wit_common_chars = []
                noise_wit_items = []   # 改為存儲 (char, idx) 元組
                
                temp_int_wit = intersection.copy()
                for (ch, w_idx), vid in zip(wit_items, wit_ids):
                    if temp_int_wit[vid] > 0:
                        wit_common_ids.append(vid)
                        wit_common_chars.append(ch)
                        temp_int_wit[vid] -= 1
                    else:
                        noise_wit_items.append((ch, w_idx))
                ...
                if base_common_ids != wit_common_ids:
                    ...
                    for vid in intersection.keys():
                        # [修正] 直接取用綁定的精確絕對索引
                        b_occurrences = [(ch, b_idx) for (ch, b_idx), v in zip(base_items, base_ids) if v == vid]
                        w_occurrences = [(ch, w_idx) for (ch, w_idx), v in zip(wit_items, wit_ids) if v == vid]
                        
                        for b_char, actual_b_idx in b_occurrences:
                            matched = False
                            for w_char, actual_w_idx in w_occurrences:
                                if b_char != w_char and self.is_variant_relation(b_char, w_char):
                                    final_blocks.append({
                                        ...
                                        'base_clean_len': 1,
                                        'wit_clean_len': 1,
                                        'idx': actual_b_idx,      # <--- 使用真實絕對索引
                                        'wit_idx': actual_w_idx   # <--- 使用真實絕對索引
                                    })
                                    matched = True
                                    break 
                            if matched:
                    ...
                    # 3. 輸出【噪聲】條目 (訛/脫/衍)
                    n_base = "".join([x[0] for x in noise_base_items])
                    n_wit = "".join([x[0] for x in noise_wit_items])
                    ...
                    if n_base or n_wit:
                        ...
                        # [核心修正]: 獲取噪聲第一個字的精確起始位置
                        exact_b_idx = noise_base_items[0][1] if noise_base_items else start_idx
                        exact_w_idx = noise_wit_items[0][1] if noise_wit_items else start_wit_idx
                        ...
                        final_blocks.append({
                            ...
                            'idx': exact_b_idx,      # <--- 訛誤將精確標註在具體發生錯誤的字上
                            'wit_idx': exact_w_idx
                        })
...
class WordExportThread(QThread):
    ...
    def __init__(self, output_path, base_full_text, insertion_points, available_fonts, font_map, export_format='footnote'):
        super().__init__()
    ...
    # --- 核心：智能寫入函數---
    def smart_insert_com(self, word_range, text, is_inline=False):
        """
        [技術核心]：基於 COM 的智能文本寫入。
        ...
        for font_name, content in segments:
            ...
            if is_inline:
                # 夾注樣式：紅色、楷體、小五號
                word_range.Font.NameFarEast = "楷体"
                word_range.Font.NameAscii = "楷体"
                word_range.Font.Color = 255     # 255 是紅色
                word_range.Font.Size = 10.5    # 夾注字號
                word_range.Font.Bold = False    # 確保不加粗
            else:
                # 正文樣式：【必須顯式重置】
                word_range.Font.Color = 0       # 重置為黑色 (wdColorBlack)
                word_range.Font.Size = 12      # 重置為正文字號 (小四)
                word_range.Font.Bold = False    # 確保不加粗
    ...
    def run(self):
        ...
        try:
            ...
            for i, note_idx in enumerate(sorted_indices):
                ...
                if self.export_format == 'footnote':
                    # 腳註模式
                    ...
                    rng = doc.Content
                    rng.Collapse(0)
                    self.smart_insert_com(rng, combined_note, is_inline=True)
...
class CustomDictWindow(QDialog):
    ...
    def init_ui(self):
        ...
        info = QLabel("【說明】左側定義【自定義異體】(如 A=B)，右側定義【自定義訛文】(如 A!=B)。\n"
                      ...
                      "規則優先級高於系統內置臺灣地區“教育部”異體字字典（第七版），您可以利用該版塊處理一些因刻工習慣或寫本特徵出現的特殊情況。\n"
                      )
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        line_preview = QFrame(); line_preview.setFrameShape(QFrame.Shape.VLine); line_preview.setFrameShadow(QFrame.Shadow.Sunken)
        t4_toolbar.addWidget(line_preview)

        self.radio_footnote = QRadioButton("腳註預覽")
        self.radio_footnote.setChecked(True)
        self.radio_inline = QRadioButton("夾注預覽")
        
        self.preview_style_group = QButtonGroup(self)
        self.preview_style_group.addButton(self.radio_footnote)
        self.preview_style_group.addButton(self.radio_inline)
        
        self.radio_footnote.toggled.connect(self.refresh_note_preview)
        self.radio_inline.toggled.connect(self.refresh_note_preview)
        
        t4_toolbar.addWidget(self.radio_footnote)
        t4_toolbar.addWidget(self.radio_inline)
    ...
    def add_wit(self):
        ...
        if paths:
            for p in paths:
                self.list_wits.addItem(os.path.basename(p))
                self.list_wits.item(self.list_wits.count()-1).setData(Qt.ItemDataRole.UserRole, p)
            self.check_ready()
...
# --- 信號處理 ---
    ...
    def show_note_context_menu(self, pos):
        ...
        # 判斷是否顯示批量刪除
        # 邏輯：不包含「衍、脫、倒」關鍵字的，視為可能是異體或訛誤，允許批量刪除
   ...
   # ==========================================
    ...
    def refresh_note_preview(self):
        ...
        is_inline_mode = self.radio_inline.isChecked() # 獲取當前預覽模式
        ...
        for rec in self.note_records:
            ...
            display_map[rec['pos_idx']].append({
                'num': current_num, 
                'html': display_text, 
                'raw': content
            })
        ...
        while cursor <= len(self.base_full_text):
            ...
            if cursor in display_map:
                ...
                # 如果是夾注預覽模式
                if is_inline_mode:
                    # 將同一個位置的多條校勘記純文本合併在一起
                    combined_raw = "；".join([n['raw'] for n in notes]) # 用分號隔開多條
                    # 插入紅色楷體括號
                    html += f"<span style='color:#B74639; font-family:KaiTi;'>[{combined_raw}]</span>"
                
                # 如果是傳統腳註模式
                else:
                    nums = [str(n['num']) for n in notes]
                    nums_str = ",".join(nums)
                    html += f"<sup style='color:#3E5E79; font-weight:bold; font-size:12px;'>{nums_str}</sup>"
                    # 收集底部注釋內容
                    for n in notes:
                        page_notes_html.append(f"<p style='margin:5px 0;'>{n['num']}. {n['html']}</p>")
        ...
        # 如果是腳註模式，而且確實有校勘記，在底部畫分割線和列表
        if not is_inline_mode and page_notes_html:
            html += "<hr style='border:1px solid #D0D0D0; margin:20px 0;'>"
            html += "<div style='font-size:14px; color:#555;'>"
    ...
    def export_real_footnote(self):
        ...
        msg_box.setWindowTitle("選擇導出格式")
        msg_box.setText("請選擇校勘記在 Word 中的排版格式：")
        btn_footnote = msg_box.addButton("腳註校勘記", QMessageBox.ButtonRole.AcceptRole)
        btn_inline = msg_box.addButton("夾注校勘記", QMessageBox.ButtonRole.AcceptRole)
        msg_box.addButton("取消", QMessageBox.ButtonRole.RejectRole)
        msg_box.exec()
        
        if msg_box.clickedButton() == btn_footnote:
            export_format = 'footnote'
        elif msg_box.clickedButton() == btn_inline:
            export_format = 'inline'
        else:
            return  # 用戶點擊取消

        if export_format == 'footnote':
            default_filename = "底本附校勘記(腳註形式).docx"
        else:
            default_filename = "底本附校勘記(夾注形式).docx"

        # 將原本寫死的 "底本附校勘記_修訂版.docx" 替換為 default_filename 變數
        path, _ = QFileDialog.getSaveFileName(self, "導出原文附校勘記", default_filename, "Word (*.docx)")
        if not path: return
        
        self.btn_export_word_note.setEnabled(False)
        self.lbl_status.setText("正在導出修訂後的校勘記...")
        
        self.word_thread = WordExportThread(path, self.base_full_text, final_insertion_points, self.available_font_families, self.font_map, export_format)
